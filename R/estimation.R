  ...
  function(names = c("Fm", "Winf", "Wfs"),
    ...
    if (is(data, "list")) {
      if ("df" %in% names(data)) {
        ...
        stop("`data` is a list not containing an element named sample or df.")
      }
    }
    ...
    estim <- nlminb(log(start), minimizeme,
    ...
    if (verbose) print(ci)
    return(structure(p,
      par = res, hessian = h, jacobian = s, st.er = st.er, ci = ci,
...
estimateMultidata <-
  function(names = c("Fm", "Winf", "Wfs"),
    ...
    estim <- nlminb(log(start), minimizemeMultidata,
      surdata = surdata, comdata = comdata,
...
estimate_TMB <- function(df, n = 0.75, epsilon_a = 0.8, epsilon_r = 0.1, A = 4.47,
  ...
  isTS <- is(df, "list")
  if (isTS) {
    if (is.null(totalYield)) {
      totalYield <- rep(0.01234567, length(df))
    }
  ...
  tryer <- try(
    {
      ...
      data <- list(
        binsize = binsize, nwc = nwc, freq = freq, n = n, epsilon_a = epsilon_a,
      ...
      pars <- list(
        loga = loga, logFm = logFm, logWinf = log(Winf),
