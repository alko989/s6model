...
downloadExchange("NS-IBTS", 2000:2014) # Make sure the directory doesn't have other DATRAS files, else an error occurs
## Read in all data
...
addSpatialData <- function(d,shape,select=NULL,...){
  require(maptools) # Here you define packages
  if(is.character(shape)){
    if(file.exists(shape)){
      # shape <- sf::st_read(shape) # Function deprecated
      # 
      shape <- rgdal::readOGR(shape)
    }
  }
  i <- complete.cases(d[[2]][c("lon","lat")]) # Check where there are complete lon and lat cases
  tmp <- d[[2]][i,,drop=FALSE]
  coordinates(tmp) <- ~lon+lat # use those coordinates 
 
  # Set all in same format
  CRS.new <-  proj4string(shape) 
  
  proj4string(tmp) <- CRS.new 
  proj4string(shape) <- CRS.new
...
# Remove absolute connection to PC
shape = "C:/Users/berth/Google Drive/PhD/Projects/Project 1/Data/ICES_areas/ICES_Areas_20160601_cut_dense_3857.shp"
d = nscod

## Add area information
nscod <- addSpatialData(nscod, shape = "C:/Users/berth/Google Drive/PhD/Projects/Project 1/Data/ICES_areas/ICES_Areas_20160601_cut_dense_3857.shp", select = NULL) # addSpatialData needs to be updated.
