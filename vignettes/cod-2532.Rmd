...
Since it is an assessment using only survey data, we are using `isSurvey = TRUE` and we make the assumption that all sizes are selected by the gear by setting the relative selectivity parameter ($\eta_S$) of the gear very small.
```{r, echo = TRUE, eval = FALSE}
res <- makeAssessment(ebcoddat, fnout = NULL, isSurvey = TRUE, nsample = 10, eta_S = 0.000000001, a.mean = 0.22, a.sd = 0.7)
