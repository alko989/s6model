...
#'
#' @param Winf numeric, asymptotic weight \eqn{W_\infty}
#' @param Fm numeric, fishing mortality
#' @param A numeric, growth parameter
#' @param n numeric, exponent of consumption
#' @param eta_F numeric, 50\% retention size, relative to \eqn{W_\infty}
#' @param eta_m numeric, 50\% maturation size, relative to \eqn{W_\infty}
#' @param eta_S numeric, 50\% retention size (survey), relative to \eqn{W_\infty}
...
s6params <- function(
  ...
  if (usebase && ! is.s6params(base))
    stop("base should be an s6params object", call. = FALSE)

  for (arg in userargs) {
    if (length(eval(parse(text = arg))) != 1)
      stop(paste(arg, " should not be a vector"), call. = FALSE)
    if (! is.finite(eval(parse(text = arg))))
      stop(paste(arg, " should be finite number"), call. = FALSE)
  }
  ...
  e$.__env__ <- e
  structure(e, class = "s6params", s6version = getVersion("s6model"))
}
...
add_clone_fun <- function(envir) {
  function() {
    ...
    structure(res, class = "s6params", s6version = attr(envir, "s6version"))
  }
}
...
#' @param isSurvey logical, if \code{TRUE}, simulate survey data
#'
#' @note In \code{simulate}, all zero bins after the last non-zero bin are dropped; even if \code{keepZeros} is TRUE.
#'
#' @export
#' @rdname s6params
simulate.s6params <- function(object, nsim = 1000, seed = NULL, binsize = 100,
                              keepZeros = TRUE, ndataset = 1, isSurvey = FALSE, ...) {
  ...
  s6input(wf = res, surwf = NULL, isSimulated = TRUE, trueParameters = object,
          catch = data.frame(Year = seq(ndataset), Catch = rep(1, ndataset)), ...)
}
